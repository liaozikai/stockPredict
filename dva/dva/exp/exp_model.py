...
class Exp_Model(object):
    ...
    def test(self, setting):
        ...
        ic, rank_ic, icir, rank_icir = self.calculate(preds, trues)
        ...
        return mae, mse, ic, icir, rank_ic, rank_icir

    def calculate(self, preds, trues):
        # 将预测值和真实值转换为二维数组
        preds_flat = preds.reshape(-1, preds.shape[-1]).flatten()
        trues_flat = trues.reshape(-1, trues.shape[-1]).flatten()

        # 计算平均值
        y_mean = np.mean(trues_flat)
        y_pred_mean = np.mean(preds_flat)

        # 计算标准差
        y_std = np.std(trues_flat)
        y_pred_std = np.std(preds_flat)

        # 计算IC
        num = np.sum((trues_flat - y_mean) * (preds_flat - y_pred_mean))
        denom = np.sqrt(np.sum((trues_flat - y_mean) ** 2)) * np.sqrt(np.sum((preds_flat - y_pred_mean) ** 2))
        ic = abs(num / denom)

        # 计算Rank IC
        rank_y = np.argsort(np.argsort(trues_flat))
        rank_y_pred = np.argsort(np.argsort(preds_flat))
        rank_ic_num = np.sum((rank_y - np.mean(rank_y)) * (rank_y_pred - np.mean(rank_y_pred)))
        rank_ic_denom = np.sqrt(np.sum((rank_y - np.mean(rank_y)) ** 2)) * np.sqrt(
            np.sum((rank_y_pred - np.mean(rank_y_pred)) ** 2))
        rank_ic = abs(rank_ic_num / rank_ic_denom)

        # 计算ICIR和Rank ICIR
        # 为了计算ICIR和Rank ICIR，我们需要一个随机预测的IC和Rank IC作为基准
        # 这里我们假设随机预测的IC和Rank IC为0（最差情况），实际应用中应使用适当的随机预测基准
        ic_random = 0
        rank_ic_random = 0

        icir = abs((ic - ic_random) / (1 - ic_random))
        rank_icir = abs((rank_ic - rank_ic_random) / (1 - rank_ic_random))
