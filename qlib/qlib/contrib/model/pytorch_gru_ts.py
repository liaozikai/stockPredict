...
class GRU(Model):
    ...
    def test_epoch(self, data_loader):
        ...
        for data, weight in data_loader:
            ...
            with torch.no_grad():
                ...
                # mse = self.metric_fn(pred, label)
                # mae = self.metric_fn(pred, label)
    ...
    def predict(self, dataset):
        print("===========gru_ts_mse_single_predict=========")
        if not self.fitted:
        ...
        print("===========gru_ts_mse_single_01=========")
        for data in test_loader:
            feature = data[:, :, 0:-1].to(self.device)
            labelTensor = data[:, -1, -1].to(self.device)
