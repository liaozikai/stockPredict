...
class LSTM(Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.logger.info("model:\n{:}".format(self.lstm_model))
        self.logger.info("model size: {:.4f} MB".format(count_parameters(self.lstm_model)))
    ...
    def predict(self, dataset: DatasetH, segment: Union[Text, slice] = "test"):
        ...
        y_values = np.squeeze(y_test.values)
        ...
        for begin in range(sample_num)[:: self.batch_size]:
            ...
            labelTensor = torch.from_numpy(y_values[begin:end]).float().to(self.device)

            with torch.no_grad():
                pred = self.lstm_model(x_batch).detach().cpu().numpy()
                label = labelTensor.cpu().numpy()
                preds.append(pred)

                # 选择有效的数据点
                valid_indices = ~np.isnan(label)
                ...
                mse = np.mean((valid_pred - valid_label) ** 2)
                print(mse)
                mae = np.mean(np.abs(valid_pred - valid_label))
                print(mse)

            mses.append(mse)
            maes.append(mae)

        print("===========np mean mses values:=========")
        print(np.mean(mses))
