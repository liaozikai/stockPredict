...
class HIST(nn.Module):
    ...
    def forward(self, x, concept_matrix, market_value):
        ...
        x_hidden = x.reshape(len(x), self.d_feat, -1) # [N, F, T]  x_hidden 的形状变为 [N, F, T]，其中 N 是数据点的数量，F 是特征数，T 是时间步长或其他序列长度。
        x_hidden = x_hidden.permute(0, 2, 1) # [N, T, F]
        x_hidden, _ = self.rnn(x_hidden)
        x_hidden = x_hidden[:, -1, :] #x_hidden 将只包含序列中最后一步的特征表示，这可以用于后续的计算或作为模型的输出。
