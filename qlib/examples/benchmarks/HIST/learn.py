...
from utils import metric_fn, mse, mae
...
def loss_fn(pred, label, args):
    if pred.numel() == 1 and ~torch.isnan(label):
...
def train_epoch(epoch, model, optimizer, train_loader, writer, args, stock2concept_matrix = None):
    ...
    model.train()
    print("model size: {:.4f} MB".format(count_parameters(model)))
...
def test_epoch(epoch, model, test_loader, writer, args, stock2concept_matrix=None, prefix='Test'):
    ...
    return np.mean(losses), scores, precision, recall, ic, rank_ic, np.mean(maes)
...
def inference(model, data_loader, stock2concept_matrix=None):
    ...
    infer_time = total_infer_time/len(preds)
    preds = pd.concat(preds, axis=0)
    print('==========total_infer_time===========')
    print(total_infer_time)
    print('==========infer_time===========')
    print(infer_time)
    return preds
...
def create_loaders(args):
    ...
    # df_market_value = df_market_value/1000000000  #CSI300
    df_market_value = df_market_value/1000000000  #CSI500
    ...
    # df_train['stock_index'] = 839 #CSI300
    df_train['stock_index'] = 1307 #CSI500
    # df_train['stock_index'] = df_train.index.get_level_values('instrument').map(stock_index).fillna(839).astype(int) #CSI300
    df_train['stock_index'] = df_train.index.get_level_values('instrument').map(stock_index).fillna(1307).astype(int) #CSI500
    ...
    df_valid['stock_index'] = 839
    # df_valid['stock_index'] = 1307
    # df_valid['stock_index'] = df_valid.index.get_level_values('instrument').map(stock_index).fillna(839).astype(int)
    df_valid['stock_index'] = df_valid.index.get_level_values('instrument').map(stock_index).fillna(1307).astype(int)
    ...
    df_test['stock_index'] = 839
    # df_test['stock_index'] = 1307
    df_test['stock_index'] = df_test.index.get_level_values('instrument').map(stock_index).fillna(839).astype(int)
    # df_test['stock_index'] = df_test.index.get_level_values('instrument').map(stock_index).fillna(1307).astype(int)
...
def main(args):
    ...
    for times in range(args.repeat):
        ...
        for epoch in range(args.n_epochs):
            ...
            pprint('evaluating...')
            train_loss, train_score, train_precision, train_recall, train_ic, train_rank_ic, train_mae = test_epoch(epoch, model, train_loader, writer, args, stock2concept_matrix, prefix='Train')
            val_loss, val_score, val_precision, val_recall, val_ic, val_rank_ic, val_mae = test_epoch(epoch, model, valid_loader, writer, args, stock2concept_matrix, prefix='Valid')
            test_loss, test_score, test_precision, test_recall, test_ic, test_rank_ic, test_mae = test_epoch(epoch, model, test_loader, writer, args, stock2concept_matrix, prefix='Test')
        ...
        for name in ['train', 'valid', 'test']:
            ...
            res[name+'-IC'] = ic
            res[name+'-ICIR'] = ic.mean() / (ic.std()+EPS)
            res[name+'-RankIC'] = rank_ic
            res[name+'-RankICIR'] = rank_ic.mean() / (rank_ic.std()+EPS)
    ...
    pprint(('IC: %.4f (%.4f), Rank IC: %.4f (%.4f), ICIR: %.4f (%.4f), RANK ICIR: %.4f (%.4f)')%(np.array(all_ic).mean(), np.array(all_ic).std(), np.array(all_rank_ic).mean(), np.array(all_rank_ic).std()))
...
def parse_args():
    ...
    parser.add_argument('--data_set', type=str, default='csi500')
    parser.add_argument('--pin_memory', action='store_false', default=True)
    parser.add_argument('--batch_size', type=int, default=-1) # -1 indicate daily batch
    parser.add_argument('--least_samples_num', type=float, default=1137.0) 
    parser.add_argument('--label', default='') # specify other labels
    # parser.add_argument('--train_start_date', default='2005-04-08')  #CSI300
    parser.add_argument('--train_start_date', default='2007-01-31')  #CSI500
    ...
    parser.add_argument('--test_end_date', default='2023-12-29')
    ...
    parser.add_argument('--name', type=str, default='csi500_HIST')


    # input for csi 300
    # parser.add_argument('--market_value_path', default='./data/market_value_csi300.pkl')
    # parser.add_argument('--stock2concept_matrix', default='./data/stock2concept_300.npy')
    # parser.add_argument('--stock_index', default='./data/stock_index_300.npy')

    # input for csi 500
    parser.add_argument('--market_value_path', default='./data/market_value_csi500.pkl')
    parser.add_argument('--stock2concept_matrix', default='./data/stock2concept_500.npy')
    parser.add_argument('--stock_index', default='./data/stock_index_500.npy')


    # parser.add_argument('--outdir', default='./output/csi300_HIST')
    parser.add_argument('--outdir', default='./output/csi500_HIST')
