...
from qlib.contrib import torch
...
from incremental.model import MetaModelInc, DoubleAdaptManager
from incremental.dataset import MetaDatasetInc
from incremental.utils import *
...
class Incremental:
    ...
    def dump_data(self):
        ...
        data_warpper = ds.prepare("train", col_set=["feature", "label"], data_key=DataHandlerLP.DK_L)
        if t["dataset"]["class"] == "TSDatasetH":
            data = data_warpper.data
        else:
            data = data_warpper
        ...
        if t["dataset"]["class"] == "TSDatasetH":
            ...
            data_warpper.config(fillna_type="ffill+bfill")  # process nan brought by dataloader

        ta = TimeAdjuster(future=True, end_time=segments['test'][1])
        print(ta.align_seg(t["dataset"]["kwargs"]["segments"]["train"])[0])
        ...
        md_offline = MetaDatasetInc(data=data_warpper, **kwargs)
        ...
        md_online = MetaDatasetInc(data=data_warpper, data_I=data_I, **kwargs)
    ...
    def offline_training(self, seed=43):
        ...
        #         data_dir=self.data_dir,
        #         market=self.market,
        #         model_type=selff.forecast_model,
        #         alpha=self.alpha,
        #         rank_label=self.rank_label,
        #         h_path=self.h_path,
    ...
    def online_training(self, meta_tasks_test, meta_model, tag=""):
        ...
        rec = self.backtest(pred_y_all, start_time)
        return rec

    def backtest(self, pred_y_all, start_time):

        print("===========pred_y_all=========: ", pred_y_all)
        ...
        end_time = time.time()
        print("===========len(pred)=========: ", len(pred_y_all))

        print("===========infer time: ", end_time - start_time)
        print("===========single infer time: ", (end_time - start_time) / len(pred_y_all))
