...
def get_data_and_idx(data: TSDataSampler, segment: List):
    if segment[1] == 'None':
        print("========None==========")
        return None, None  # 或者返回其他你认为合适的值
...
def preprocess(
    ...
    for i, task in enumerate(task_list):
        if not hasattr(task, 'processed_meta_input'):
            continue
    ...
    if skip_ids:
        ...
        while i < len(skip_ids):
            task_w_train_wo_test = new_task_list[skip_ids[i]]
            ...
            if not_skip < len(new_task_list):
                for key in task_w_train_wo_test.keys():
                    if 'train' in key:
                        new_task_list[not_skip][key] = task_w_train_wo_test[key]
            i += 1

        new_task_list = [new_task_list[i] for i in range(len(new_task_list)) if i not in skip_ids]
